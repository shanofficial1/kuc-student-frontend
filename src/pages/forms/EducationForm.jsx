      ...
      <FormSection title="Academic Records" icon={School}>
        <div className="md:col-span-2 space-y-6">
          {records.map((record, index) => {
            const isStaticType = ['10th', 'Plus Two'].includes(record.qualType);
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <SelectField
                    ...
                    options={getQualificationOptions(record.qualType)}
                    onChange={(e) => updateAcademicField(index, 'qualType', e.target.value)}
                    disabled={isSubmitted}
                  />

                  {record.qualType === 'Ph.D' ? (
                    renderPhdFields(record, index)
                  ) : (
                    <>
                      {(!record.qualType || ['10th', 'Plus Two', 'UG', 'PG', 'M.Phil'].includes(record.qualType)) && 
                        renderBaseFields(record, index)
                      }
                      {['UG', 'PG', 'M.Phil'].includes(record.qualType) && (
                        <>
                          <InputField
          ...
          {!isSubmitted && (
            <button
              type="button"
              onClick={() => updateSection('education', { education: [...records, { qualType: '' }] })}
            ...
            <div key={index} className="p-4 bg-white border border-slate-200 rounded-xl space-y-4 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InputField 
                  label="Exam Name" 
                  value={exam.examName || ''} 
                  onChange={(e) => updateExamField(index, 'examName', e.target.value)} 
                  disabled={isSubmitted} 
                />
                <InputField 
                  label="Score" 
                ...
                <SelectField 
                  ...
                  onChange={(e) => updateExamField(index, 'year', e.target.value)} 
                  disabled={isSubmitted} 
                />
